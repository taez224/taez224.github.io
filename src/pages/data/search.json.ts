...
export const GET: APIRoute = async () => {
  ...
  const records = [
    ...garden.notes.map((note) => ({ kind: note.kind, label: kindLabel(note), url: note.url, title: cleanTitle(note.title), summary: note.summary, tags: note.publicTags.map(displayTag), headings: note.headings.map((h) => h.title), text: note.bodyText })),
