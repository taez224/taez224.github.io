import { KINDS, type NoteKind } from './kinds.ts';
...
export function slugFor(meta: { slug?: unknown } | null | undefined, title: unknown): string {
...
export function kindPrefix(kind: NoteKind): string {
  const prefix = KINDS[kind]?.prefix;
  if (!prefix) throw new Error(`Unknown note kind: ${kind}`);
  return prefix;
}

export function noteUrl(basePath: string | null | undefined, kind: NoteKind, slug: string, fragment: string = ''): string {
...
export function siteHome(site: string | URL, basePath: string | null = ''): URL {
  return new URL(`${String(basePath ?? '').replace(/\/$/, '')}/`, site);
}

export function assertUniqueSlugs(entries: readonly { kind: NoteKind | 'book'; slug: string; path: string }[]): void {
