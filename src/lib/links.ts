...
function stripLinkTarget(rawTarget: string): string {
  return rawTarget.split('|')[0].split('#')[0].trim().replace(/^!/, '');
}

// 같은 열쇠에 값을 모은다. 붙일 때마다 배열을 통째로 복사하지 않는다.
export function addTo<K, V>(map: Map<K, V[]>, key: K, value: V): void {
...
export function indexByBasename(paths: Iterable<string>): Map<string, string[]> {
  const index = new Map<string, string[]>();
  for (const relativePath of paths) addTo(index, path.posix.basename(relativePath).toLowerCase(), relativePath);
  return index;
}

interface PathLookup { has(path: string): boolean }

export function resolveTarget(sourcePath: string, rawTarget: string, byPath: PathLookup, byBasename: ReadonlyMap<string, readonly string[]>, preferred: PathLookup | null = null): string | null {
...
export function noteTargets(body: string, related: unknown = []): string[] {
...
export function resolveTargets(sourcePath: string, targets: readonly string[], byPath: PathLookup, byBasename: ReadonlyMap<string, readonly string[]>): string[] {
  return [...new Set(targets.map((target) => resolveTarget(sourcePath, target, byPath, byBasename)).filter((target): target is string => Boolean(target)))];
}
