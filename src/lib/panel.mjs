import { kindLabel, formatDate, displayTag, topicColor } from './format.mjs';

export function panelModel(node, notesByPath, noteEdges) {
  const ref = (path) => { const n = notesByPath.get(path); return n ? { title: n.displayTitle || n.title, url: n.url } : null; };
  ...
  return {
    ...
    title: node.displayTitle || node.title, url: node.url, topics,
