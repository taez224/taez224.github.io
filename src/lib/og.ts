...
import { kindLabel, formatDate, cleanTitle, escapeXml as esc } from './format.ts';
...
const RENDER_OPTIONS = { fitTo: { mode: 'width' as const, value: CARD.width } };
...
const isCard = (png: unknown) => { const d = pngDimensions(png); return Boolean(d && d.width === CARD.width && d.height === CARD.height); };
...
export async function cachedPng(key: string, render: RenderPng): Promise<Buffer> {
...
async function readOrRender(key: string, render: RenderPng): Promise<Buffer> {
...
let resvgVersionPromise: Promise<string> | null = null;
...
let fontsPromise: Promise<{ files: string[]; meta: { name: string; size: number }[] } | null> | null = null;
...
function renderPng(svg: string, fontFiles: string[] | null): Buffer {
...
export function fitTitle(title: string, { maxWidth = 620, maxLines = 3, sizes = [60, 54, 48, 42, 36] } = {}) {
...
function thumbnailSourcePath(thumbnail: string | null | undefined, vaultRoot: string): string | null {
...
export async function thumbnailDataUri(thumbnail: string | null | undefined, { vaultRoot = projectPaths().vaultRoot } = {}) {
  ...
  const image = await imageService.transform(source, { src: sourcePath, width: 800, height: 800, fit: 'contain', format: 'png' }, { service: { entrypoint: 'astro/assets/services/sharp', config: {} }, endpoint: { route: '/_image' }, dangerouslyProcessSVG: false, domains: [], remotePatterns: [], responsiveStyles: false }, console);
...
export function siteLabelFor(config: { basePath?: string }, site?: string | URL): string {
...
function siteSvg({ garden, title, siteLabel }: { garden: OgGarden; title: string; siteLabel: string }): string {
...
export async function renderSiteOgPng(garden: OgGarden, { title, siteLabel }: { title: string; siteLabel: string }): Promise<Buffer> {
