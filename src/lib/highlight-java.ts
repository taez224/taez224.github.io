...
type Pattern = { className: HighlightTokenClass | ((match: RegExpExecArray) => HighlightTokenClass); regex: RegExp };
...
export const java = defineLanguage({
  ...
  tokenize: (code) => runPatterns(code, [
    ...
    { className: 'meta', regex: new RegExp(String.raw`@${IDENTIFIER_START}${IDENTIFIER_PART}*(?:\.${IDENTIFIER_START}${IDENTIFIER_PART}*)*`, 'gu') },
