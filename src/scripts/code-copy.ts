...
// 복사하면 2초 동안 체크를, 실패하면 5초 동안 안내를 보인다.
...
export function setupCodeCopy(root: ParentNode = document): void {
  for (const block of root.querySelectorAll<HTMLElement>('.code-block')) {
    ...
    const button = head.querySelector<HTMLButtonElement>('.code-copy') ?? document.createElement('button');
    button.type = 'button';
    ...
    const status = head.querySelector<HTMLElement>('.code-copy-status') ?? document.createElement('span');
    status.className = HIDDEN_STATUS;
    status.setAttribute('role', 'status');
    if (!head.querySelector('.code-copy-status')) head.append(status);
    if (!head.querySelector('.code-copy')) head.append(button);
