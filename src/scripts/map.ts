import type { GardenGraphNode, PanelData } from '../lib/content-model.ts';
import type { InlineGraphData } from '../lib/graph-data.ts';
type MapData = InlineGraphData<GardenGraphNode, 'path' | 'mapKey'> & PanelData;
type PanelModel = ReturnType<typeof panelModel>;
type PanelReference = PanelModel['outgoing'][number] & { nodeId?: string; isHub?: boolean };
type DecoratedPanel = Omit<PanelModel, 'outgoing' | 'incoming'> & { outgoing: PanelReference[]; incoming: PanelReference[] };

import { createGraph, isFilteredOut } from '../graph/engine.ts';
import { layoutGraph, ATLAS_LAYOUT } from '../graph/layout.ts';
import { panelModel } from '../lib/panel.ts';
import { setupScrollFades } from './scroll-fades.ts';
import { escapeHtml as escape } from '../lib/format.ts';
import { REF_ICONS } from '../lib/ref-icons.ts';
import { loopTabFocus } from './focus-loop.ts';

const page = document.querySelector<HTMLElement>('.map-page')!;
const svg = document.querySelector<SVGSVGElement>('svg[data-map]')!;
const panel = document.querySelector<HTMLElement>('[data-panel]')!;
const body = document.querySelector<HTMLElement>('[data-panel-body]')!;
...
const list = (icon: string, label: string, items: PanelReference[]) => items.length ? `<section class="list-block"><div class="meta">${icon}${label}<span class="count">${items.length}</span></div><div class="scroll-list${items.length > 6 ? ' is-long' : ''}"><ul class="side-list">${items.map((i) => `<li><a href="${escape(i.url)}"${i.nodeId ? ` data-node="${escape(i.nodeId)}"` : ''}>${i.isHub ? '<i class="hub-mark" aria-hidden="true"></i>' : ''}${escape(i.title)}</a></li>`).join('')}</ul></div></section>` : '';

let cleanupScrollFades = () => {};
function renderPanel(model: DecoratedPanel) {
  cleanupScrollFades();
...
const data = JSON.parse(page.querySelector('[data-map-data]')!.textContent!) as MapData;
...
const decorate = (i: PanelModel['outgoing'][number]) => { const node = nodeByPath.get(i.path); return { ...i, nodeId: node?.id, isHub: node?.type === 'hub' }; };
const withNodeIds = (model: PanelModel) => ({ ...model, outgoing: model.outgoing.map(decorate), incoming: model.incoming.map(decorate) });
...
function select(id: string | null, pushUrl: boolean, { open = true } = {}) {
...
const grip = panel.querySelector<HTMLElement>('[data-sheet-grip]');
