...
export function renderSnapshotSvg(nodes: readonly GraphNode[], edges: readonly GraphEdge[], positions: ReadonlyMap<string, Point>, { width, height, preset = 'mobile', pixelHeight = 500, font = 14, regionFont = 16, compactRegionFont = 0, strokeWidth = 1.1, label = '생각 지도' }: SnapshotOptions): string {
  ...
  // 제목은 점이 아니라 허브 고리 아래에 둔다. 점 아래에 두었더니 제목 윗부분이 고리에 닿았다.
  const radiusOf = (node: GraphNode) => nodeRadius(node.degree ?? 0);
  const labelBoxes: Box[] = [], labels: { node: GraphNode; title: string; x: number; y: number }[] = [];
  ...
  for (const node of titled) {
    const p = at(node.id), title = cleanTitle(node.displayTitle ?? node.title ?? ''), w = estimateTextWidth(title, font), y = p.y + ringedRadius(node, radiusOf(node)) + font + 4;
  ...
  for (const node of nodes) {
    ...
    if (node.type === 'hub') out += `<circle cx="${p.x}" cy="${p.y}" r="${(r + RING_GAP.hub).toFixed(1)}" fill="none" stroke="var(--accent)" stroke-width="1.2" stroke-opacity=".7"></circle>`;
...
// 엔진의 hero 모드와 같은 값: nodeScale 0.9, 제목 13px, 영역 이름 15px, 간선 1.1(농도 .28), 색면 .07, 허브 링과 진입점 후광(RING_GAP).
// 제목은 엔진처럼 고리 밖에 놓는다(ringedRadius).
...
function renderDesktop(nodes: readonly GraphNode[], edges: readonly GraphEdge[], positions: ReadonlyMap<string, Point>, { width, height, pixelHeight, label }: Size & { pixelHeight: number; label: string }): string {
  const at = (id: string) => positions.get(id)!;
  ...
  const labelRadius = (node: GraphNode) => ringedRadius(node, radiusOf(node));
  ...
  const obstacles = [
    ...placed.map((node) => nodeBox(at(node.id), labelRadius(node) + 2 * u)),
  ...
  for (const node of placed) {
    ...
    if (node.isEntry) out += `<circle cx="${p.x}" cy="${p.y}" r="${(r + RING_GAP.entry).toFixed(1)}" fill="var(--accent-soft)" stroke="var(--accent)" stroke-width="1" stroke-opacity=".5"></circle>`;
