...
import { imageMimeType } from '../lib/image-types.ts';
...
export function decodedAssetTargetPath(outputRoot: string, destinationRelativePath: string): string {
...
async function copyReviewedAssets({ assetCopies, vaultRoot, outputDir }: { assetCopies: ReadonlyMap<string, string>; vaultRoot: string; outputDir: string }): Promise<void> {
...
export function resolveDevAssetRequest({ assetCopies, base, pathname }: { assetCopies: ReadonlyMap<string, string>; base: string; pathname: string }): string | null {
...
function createDevAssetMiddleware({ vaultRoot, base }: { vaultRoot: string; base: string }): Connect.NextHandleFunction {
  return (req, res, next) => {
...
export default function vaultAssets(): AstroIntegration {
  let base = '';
  return {
    name: 'vault-assets',
    hooks: {
