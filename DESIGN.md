...
- **도표:** Mermaid는 필요한 페이지에서 지연 로딩한다. 배치 `dagre`, 외형 `neo`, 라벨 접힘 폭 `120px`을 기본으로 쓰며 값은 [mermaid-config.ts](src/scripts/mermaid-config.ts)에 모아 둔다. 본문 폭을 넘는 도표는 줄였을 때 라벨이 `13px` 이상 남으면 접어 넣고, 그보다 작아지면 원래 크기로 두고 가로로 스크롤한다. 스크롤이 생긴 도표에만 키보드 초점과 위 오른쪽 `크게 보기` 버튼을 붙이며 이름은 모든 도표에 붙인다. 모달은 전체 구조를 먼저 보여 주고 `원래 크기`로 세부를 본다. 조작은 보기 전환을 왼쪽, 닫기를 오른쪽 끝에 두고 `720px` 이하에서는 도표 아래로 내려간다. 도표를 렌더링하지 못하면 기존 코드 블록으로 원문을 표시한다.
