...
`primary`는 `--ink`와 `--accent`에 대응하며 글자와 주요 조작을 표시한다. `paper`는 페이지 배경, `paper-strong`은 표 머리·콜아웃·도표·대화상자 배경이다. 코드 상자는 강조 테마의 흰 배경(`code-bg`)을 쓴다. `muted`는 보조 정보를, `line`은 내용의 구획을 나타낸다. 보조 글자색은 `muted` 한 단계만 둔다. 종이색 위에서 4.5:1을 지키는 가장 옅은 회색이 `muted`와 거의 같아서, 개수나 단축키처럼 더 낮은 위계는 색을 옅게 하지 않고 글자 크기로 나눈다. `accent-soft`는 호버 배경에 사용하고, 필터·범례의 선택 배경은 한 단계 짙은 `line`이다(`selected-control`). `accent-soft`는 종이색과 1.10:1이라 선택 표시로는 보이지 않는다. 그 위에 개수나 연재 방향 표시 같은 `muted` 글자가 올라가므로, `muted`가 4.5:1을 넘는 범위에서 가장 짙은 값으로 두고 `selected-meta` 컴포넌트로 lint가 이 조합을 검사하게 한다. 글자를 선택하면 먹색 15% 판이 깔린다. 형광(`highlight`)은 본문 강조 표시와 구분되지 않아 선택 색으로 쓰지 않는다. `warning`, `bug`, `highlight`는 각각 `--warning`, `--bug`, `--highlight`로 연결해 본문 상태와 강조를 표현한다. 링크 글자는 먹색이라 밑줄로 주변 문장과 구분한다. 본문 링크, 홀로 선 링크(개발 노트 분류, 홈의 지도 보기와 소개 링크), 글자 버튼이 같은 먹색 35% 밑줄(`--link-underline`, 종이색에서 2.02:1)을 쓰고 호버에서 밑줄을 먹색으로 채운다. `line`(1.28:1)은 밑줄로 쓰기에는 거의 보이지 않아 구획선과 선택 판에만 쓴다.
...
`warning`과 `bug`는 본문 콜아웃의 위 괘선·머리표·아이콘에, `highlight`는 형광 표시에 사용한다. 개발은 통합된 주제색을 사용하고, 버그와 글쓰기는 같은 색상 계열을 공유하지만 용도로 구분한다. 그래프 간선은 `--edge`(`#9aab9d`) 색을 낮은 불투명도로 그리며, 투명도는 [graph.css](src/styles/graph.css)에서 관리한다. 주제색은 SVG 속성과 인라인 스타일에 들어가므로 hex 대신 `var(--topic-…)`를 참조하고, Shell이 밝은 값(`GRAPH_COLORS`)과 어두운 값(`DARK_GRAPH_COLORS`)을 변수로 싣는다. 어두운 주제색은 주제마다 밝기 단계를 달리해 색각 이상 시뮬레이션에서도 이웃 주제가 붙지 않게 골랐고, 노드 3.3:1·영역 이름 5:1 이상이다. 어두운 화면의 영역 색면은 바탕에 묻히지 않게 불투명도를 .06에서 .1로 올린다. OG 카드는 밝은 값을 직접 쓴다. 먹색의 반투명 변형(인라인 코드 배경, 링크 밑줄, 그림자, 시트 뒤 배경)은 `color-mix()`로 `--ink`에서 만들어 값을 따로 적지 않는다.

어두운 화면은 운영체제·브라우저 설정(`prefers-color-scheme: dark`)을 따르고 사이트 안에 전환 버튼은 두지 않는다. 헤더에 자리가 없고, 선택을 기억하는 스크립트가 없으니 첫 화면부터 맞는 색으로 그려진다. 값은 [palette.ts](src/lib/palette.ts)의 `DARK_PALETTE`가 단일 출처이고, YAML의 `-dark` 토큰과 `site.css`의 미디어 쿼리가 같은 값을 쓴다. 2026-09 점검 끝에 녹회색·세피아·중성 회색 세 시안을 비교해 먹색과 같은 녹회색 색조를 골랐다. 라이트를 뒤집지 않고 따로 짰다. 바탕(`paper-dark`)보다 올라온 판(`paper-strong-dark`, `accent-soft-dark`)일수록 밝아서 그림자 없이 층이 보인다. 글자는 흰색 대신 옅은 먹색이라 본문 대비가 14:1이고, `muted-dark`는 가장 밝은 판 위에서도 4.6:1이다. 먹색 반투명 변형은 `--ink`를 따라 저절로 바뀌지만, 페이지를 덮는 뒤 배경은 어둡게 해야 하므로 어두운 화면에서는 검정 반투명(검색 50%, 도표 보기 70%, 지도 시트 45%)을 따로 적는다. 검은 글리프인 GitHub·Brunch 로고는 색을 뒤집는다. 대표 글·연재·외부 발행 글·글 카드의 썸네일은 흰 바탕 그림이 밝은 덩어리로 뜨므로 밝기를 85%로 낮춘다. 색 뒤집기는 흰 바탕 일러스트에는 잘 맞지만 포스터와 사진을 망가뜨려 쓰지 않는다. 본문 안 그림과 책 표지는 그대로 둔다.

도표는 연두색 노드와 먹색 글자를 기본으로 하고, 낮은 채도의 색으로 묶음과 항목을 구분한다. 작성자가 의미를 부여한 색은 유지하며, 세부 팔레트는 [도표 설정](src/scripts/mermaid-config.ts)에서 관리한다. 어두운 화면에서는 같은 색상각에서 밝기만 다시 고른 짙은 면과 밝은 테두리(`redux-dark-color` 이름으로 사이트 값을 덮음)를 쓰고, 읽는 중에 화면 모드가 바뀌면 원문에서 다시 그린다. 앞머리로 테마를 직접 지정한 도표는 그 테마를 보여 주려는 것이라 어두운 화면에서도 지정한 색을 유지한다.
...
가로 정렬은 페이지 구조를 따른다. 목록형 페이지와 리더처럼 오른쪽에 사이드바가 있는 두 열 페이지는 워드마크와 같은 선에서 시작한다. 소개와 외부 발행 글처럼 사이드바가 없는 한 열 페이지는 `reader-column` 폭을 가운데에 둔다. 한 열 페이지를 왼쪽에 붙이면 넓은 화면에서 오른쪽이 크게 비어 미완성처럼 보이기 때문이다. 리더에서 소개로 넘어갈 때 본문 시작선이 옮겨 가지만, 두 페이지를 오가는 일이 드물어 구조에 맞는 배치를 우선한다.
...
모바일 홈은 첫 글까지의 이동을 줄이기 위해 소개 위 여백을 20px로 두고 지도를 `clamp(230px, 78vw, 350px)` 높이로 표시한다. 이 폭의 지도는 정적 그림이라 1000단위 그림이 약 1/3로 줄어 허브 제목이 7px 남짓이 되므로, 허브 제목을 숨기고 영역 이름만 크게(장면 46, 화면 약 17px) 놓은 층으로 바꾼다. 큰 이름이 그래프 한가운데로 밀려나지 않도록 그림 위쪽 여백까지 이름 자리로 쓰고, 그 여백만큼 그림 상자를 높인다. 소셜 링크는 소개 페이지에서 제공하며 데스크톱 홈에도 유지한다.
...
목록과 본문은 밝은 배경, 얇은 구분선, 여백으로 내용을 구분한다. 페이지의 판과 조작 요소에는 그림자를 쓰지 않는다(도표 안 노드의 그림자는 아래에 따로 적는다). 페이지 위에 겹치는 판은 테두리와 뒤 배경으로 층을 나눈다. 뒤 배경이 이미 층을 가르므로 그림자는 같은 단서를 한 번 더 주는 셈이고, 가는 테두리와 넓은 그림자를 함께 쓰면 경계가 흐려진다.

- 검색 대화상자와 공유 실패 안내는 페이지 위에 떠 있는 판이라 경계가 보여야 한다. 먹색 28% 테두리(`--overlay-line`)를 쓰고, 검색 대화상자 뒤에는 먹색 32% 배경을 깐다.
- 도표 크게 보기와 모바일 지도 시트는 화면 가장자리까지 덮어 판의 경계가 곧 화면 끝이다. `line` 테두리를 두고 뒤 배경은 각각 먹색 55%, 25%다.

그래프의 옅은 주제 영역은 노드의 묶음을 보여주고, 도표 노드의 옅은 그림자는 배경과 도형을 구분한다. 긴 목록과 가로 스크롤 필터의 끝에는 페이드를 두어 화면 밖에 내용이 더 있음을 알린다.

## Shapes

본문 표·코드·콜아웃은 직각, 버튼(도표 버튼 포함)·연재 탐색 링크·비공개 표시 말풍선은 4px, 검색창·공유 안내·지도 확대 컨트롤의 바깥 상자는 6px 모서리를 사용한다. 주제 점과 지도 노드는 원형이고 허브·선택 상태는 링으로 표시한다. CSS로 그린 아이콘(할 일 체크박스 2px, 비공개 표시의 자물쇠)과 모바일 시트 손잡이는 크기에 맞춘 곡률을 쓰며 토큰 대상이 아니다. 썸네일의 가장자리 처리는 이미지 유형별 스타일을 따른다. [shapes.test.ts](tests/shapes.test.ts)가 나머지 모서리를 `rounded` 토큰 값으로 제한한다.

## Components

- **버튼:** 계열은 넷이다. 상태 변화는 모두 150ms다.
  - 채운 버튼(`.btn`)은 주요 행동에 쓴다. 높이 44px, 좌우 여백 20px, 글자 15px/600이고, 작은 변형은 높이 40px, 좌우 여백 18px, 글자 14px다. 호버에서는 배경이 `muted`로 바뀐다.
  - 테두리 버튼은 본문의 도표 크게 보기와 도표 보기 도구에 쓴다. 본문 위에 홀로 놓여 경계가 필요하므로 `paper-strong` 판에 `line` 테두리를 두고, 호버·포커스에서 테두리와 글자가 먹색이 된다. 도표 보기의 닫기도 같은 도구 줄에 있어 이 계열을 따른다.
  - 투명 버튼은 헤더 아이콘, 책장 필터, 지도 범례와 확대 버튼에 쓴다. 호버에서 `accent-soft` 판을, 선택(`aria-pressed`)에서 `line` 판과 먹색 글자를 쓴다. `line` 판 위에서 보조색은 3.94:1이라 선택된 버튼 안의 개수·접힌 주제 이름도 먹색으로 올린다.
  - 글자 버튼(`.text-button`)은 검색 닫기, 모바일 지도 시트 닫기, 검색 재시도에 쓴다. `muted` 13px 글자에 밑줄을 긋고 누르는 영역을 44px로 둔다.
- **헤더 메뉴:** 현재 메뉴는 먹색 글자와, 헤더 아래 선 위에 글자 폭만큼 얹은 2px 먹색 막대로 표시한다. 글자색 차이만으로는 보조색과 2.59:1이라 흐리고, 밑줄은 호버 밑줄과 헷갈려 쓰지 않는다.
- **헤더 조작:** 검색·공유는 44px 조작 영역 안에 18px 선 아이콘을 둔다. 호버 시 밝은 배경과 먹색 아이콘으로 반응한다. 검색은 접근 가능한 이름을, 공유는 숨긴 텍스트 이름을 제공한다.
- **지도 조작:** 확대·축소·맞춤 버튼은 데스크톱 최소 36px, 모바일 최소 40px, 터치 기기에서 44px이다. 주제 필터의 선택 상태는 배경색과 `aria-pressed`로 나타낸다.
...
- **제목 아래 요약:** 개발 노트의 요약은 제목 바로 아래에 `lead` 명조 활자로 둔다. 인쇄물의 리드문처럼 선이나 판 없이 활자만으로 제목·본문과 구분한다.
- **연재 탐색:** 이전·다음 링크는 1px 테두리와 `14px 18px` 여백을 사용한다. 호버·키보드 포커스에서 먹색 테두리와 밝은 배경으로 강조한다. 연재 허브 제목 아래의 1편 안내도 같은 이동 안내라 1px `line` 테두리와 4px 모서리, `16px 18px` 여백을 쓰고 오른쪽에 채운 버튼을 둔다.
- **책 표지:** 표지를 불러오지 못하면 표지 크기의 `paper-strong` 상자에 1px `line` 테두리를 두고 책 제목을 적는다.
- **인용문:** 3px `line` 왼쪽 선과 `muted` 글자로 표시한다. 인용을 표시하는 오랜 관례라 한쪽 막대를 쓰는 곳은 인용문과 목차 레일(2px)뿐이다.
...
- **검색:** 최대 650px 대화상자에서 입력·로딩·결과·빈 상태·실패 상태를 구분한다. 입력 안내, 불러오는 중, 결과 없음, 실패는 결과 자리에서 같은 모양(`muted` 13px, `.search-status`)으로 두고 문구로 구분한다. 실패 시 글자 버튼으로 재시도를 제공하고, 단축키 안내는 운영체제에 맞게 표시한다.
...
- **바닥글:** 모든 페이지 끝에 구분선 아래 한 줄을 둔다. 왼쪽에 사이트 이름, 오른쪽에 RSS와 사이트 소개 링크가 있고, 링크는 홀로 선 링크와 같은 밑줄이다. 노트 페이지 끝 줄은 목록으로 돌아가는 링크만 둔다. 화면 높이에 맞춰 그리는 지도 페이지에는 두지 않는다.
...
- **Don't** 의미 없는 장식 요소, 글로우, 장식용 그라디언트와 그림자를 추가하지 않는다. 겹치는 판은 위 Elevation & Depth에 적은 대로 테두리와 뒤 배경으로 구분한다.
- **Don't** 주제색을 본문 문장이나 메타데이터 장식으로 확대하지 않는다.
- **Don't** 카드·목록·콜아웃·안내 상자에 1px보다 두꺼운 한쪽 막대를 두지 않는다. 구분이 필요하면 활자, 사방 테두리, 콜아웃의 위 괘선을 쓴다.
...
호버와 선택은 색·선·링으로 즉시 알아볼 수 있게 한다. 호버 효과는 올려 둘 수 있는 포인터가 있는 기기(`@media (hover: hover)`)에서만 준다. 터치 기기는 탭한 요소에 호버가 남아 대화상자를 닫은 뒤에도 버튼 판이 깔려 있기 때문이다. 포커스와 선택 상태는 호버와 같은 모양이어도 규칙을 따로 적어 미디어 쿼리 밖에 둔다. 키보드 포커스는 전역 2px 윤곽선과 4px 간격으로 표시하고, 연재 링크와 그래프는 각자의 테두리·링을 사용한다. 본문과 조작 요소의 상태 변화는 150ms, 누르는 순간 채운 버튼·아이콘 버튼·지도 확대 버튼이 97%로 줄어드는 반응은 150ms ease-out(`cubic-bezier(.23, 1, .32, 1)`), 모바일 지도 패널의 열림·닫힘은 200ms, 그래프의 선택 이동은 320ms 안에서 처리한다. 그래프의 드래그·확대는 장식용 자동 모션이 아니라 사용자의 입력에 따른 조작이다. `prefers-reduced-motion`에서는 전환과 부드러운 스크롤을 끄고, 모바일에서는 조작 영역과 주변 간격을 함께 확인한다.

누르는 영역은 보이는 크기와 나눠 다룬다. 글자와 배치는 그대로 두고 padding과 같은 양의 음수 margin을 짝지어 영역만 넓힌다. 헤더 메뉴는 헤더 높이 전체와 메뉴 간격의 절반까지를 누르는 영역으로 쓴다. 목록 행(글·개발 노트 목록, 홈 최근 기록)은 제목 링크의 영역을 행 전체로 펼치고, 행 안의 다른 링크는 그 위에 올려 따로 누르게 한다. 지도 범례, 확대 버튼, 책장 필터, 목차와 사이드 목록 줄은 `pointer: coarse`(터치)에서만 44px로 키워, 마우스 환경은 촘촘한 밀도를 유지한다. 모바일 지도 패널 닫기와 검색 닫기는 입력 방식과 상관없이 44px이다. "글"처럼 짧은 메뉴는 옆 메뉴와의 간격 때문에 폭을 44px로 넓힐 수 없어, 간격을 포함한 WCAG 2.5.8 최소 기준(24px)을 따른다.
