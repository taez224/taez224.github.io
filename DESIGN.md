...
오른쪽 끝에는 검색과 화면 모드 버튼을 44px 상자로 붙여 둔다. 18px 기호가 상자 안에서 이미 양옆으로 비어 있어, 상자 사이를 띄우면 두 기호가 메뉴 글자 사이보다 멀어진다. 테마 버튼의 기호는 다음에 일어날 일을 보인다. 밝은 화면에서는 달이다. 스크립트가 없으면 동작하지 않으므로 아예 보이지 않는다.
