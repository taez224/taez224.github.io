...
description: 한 엔지니어의 글과 노트를 읽고 지도와 참조 관계로 생각의 연결을 따라가는 개인 사이트
colors:
  primary: '#252e29'
  paper: '#f7f6f0'
  paper-strong: '#fdfcf8'
...
TaeZ's Thinking Garden은 소프트웨어 엔지니어 TaeZ의 개인 사이트다. 글과 생각 노트, 개발 노트, 읽은 책을 모으고, 주로 AI와 함께 일하는 방식, 팀과 조직, 기록과 글쓰기를 다룬다.

독자는 둘이다. 검색이나 링크로 한 편에 들어온 독자는 그 글을 읽고 참조를 따라 다른 노트로 넘어간다. 글과 노트 페이지는 이 독자에게 맞춘다. 저자가 궁금해서 온 독자는 홈과 소개에서 시작해, 이 사람이 무엇을 만들고 어떤 문제에 관심이 있으며 어떻게 판단하는지를 몇 편의 글로 확인한다. 홈과 소개는 이 독자에게 맞춘다. 두 독자 모두에게 근거는 실적 문구가 아니라 글과 노트 자체다.

화면은 인쇄된 기록장과 개인 서재의 장부를 기준으로 삼는다. 첫인상을 만드는 장식보다 긴 글의 읽기 흐름과 노트 사이의 관계를 앞세우고, 마케팅 랜딩 페이지나 데이터 대시보드처럼 한 화면을 정보로 채우지 않는다.
...
- **지도:** 1000px 이하에서는 패널을 모달 하단 시트로 띄운다. 흐름에 두면 무대 아래로 내려가 노드를 골라도 화면에서 바뀌는 것이 없다. 경계는 입력 방식이 아니라 한 열로 접히는 폭으로 정한다. 모달은 시트에 가려진 노드로 포커스가 들어가는 것을 막는다.
- **홈 첫 화면:** 첫 화면의 높이는 기준이지 한계가 아니다. 넘치는 부분을 잘라 내면 글자를 키워 쓰는 독자가 링크와 연락처를 잃는다. 720px 이하와 손가락으로 쓰는 기기는 살아 있는 지도 대신 빌드 때 그린 그림을 받는다. 살아 있는 지도는 세로 스와이프를 가져가고, 그 대가인 호버 미리보기는 손가락에 없다. 720px 이하의 그림은 허브 제목 대신 영역 이름만 크게 둔다. 낮은 휴대폰에서는 화면 높이에 맞춰 그림을 이어서 줄여 대표 글 제목이 첫 화면에 들어오게 한다. 홈이 권하는 일은 한 편을 읽는 것이라, 지도가 첫 화면을 다 차지하면 그 글을 찾으려고 스크롤해야 한다. 그림은 150px 아래로 줄이지 않으므로 320×568처럼 아주 작은 화면에서는 제목이 첫 화면 밖에 남는다. 넓은 터치 화면은 허브 제목이 있는 정적 그림을 쓰고, 두 그림 모두 전체가 지도 링크다. 소개문 아래의 보조 링크 둘은 넓은 화면에서 그림이 살아 있는 지도로 바뀌어 지도로 가는 길이 사라지기 때문에 둔다.
...
- **연결 탐색:** 참조는 실선, 역참조는 점선이고, 전체 연결은 목록으로도 읽는다.
  ...
  - 사이드바의 작은 그래프는 이웃 제목을 단어 경계에서 두 줄까지 적고, 제목이 읽히도록 이웃은 여섯까지만 그린다. 목록의 같은 줄과 함께 강조할 때는 지도처럼 점 테두리만 바꾼다. 누르는 영역에 판을 깔았더니 이웃 제목을 덮었다.
