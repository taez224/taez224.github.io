...
- **지도:** 제목 옆 집계는 필터가 남긴 범위를 말하고 선택 때문에 움직이지 않으며, 달라진 수는 낭독기에도 전한다. 1000px 이하에서는 패널을 하단 시트로 띄운다. 흐름에 두면 무대 아래로 내려가 노드를 골라도 화면에서 바뀌는 것이 없다. 이 문제는 입력 방식이 아니라 한 열로 접히는 데서 오므로 경계는 폭으로만 정한다. 시트는 모달이다. 배경막을 걷으면 탭이 시트에 가려진 노드로 들어간다.
