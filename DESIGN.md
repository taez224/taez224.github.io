...
- **홈 첫 화면:** 첫 화면의 높이는 기준이지 한계가 아니다. 넘치는 부분을 잘라 내면 글자를 키워 쓰는 독자가 링크와 연락처를 잃는다. 720px 이하와 손가락으로 쓰는 기기는 살아 있는 지도 대신 빌드 때 그린 그림을 받는다. 살아 있는 지도는 세로 스와이프를 가져가고, 그 대가인 호버 미리보기는 손가락에 없다. 이 그림은 허브 제목 대신 영역 이름만 크게 두고 그림 전체가 지도 링크다. 소개문 아래의 보조 링크 둘은 넓은 화면에서 그림이 살아 있는 지도로 바뀌어 지도로 가는 길이 사라지기 때문에 둔다.
