...
도표 팔레트는 한 도표 안에서 묶음과 항목을 구분하는 열두 칸이며 주제색과 같은 대역(채도 15~38%, 명도 43~52%)에서 고른다. 채움은 같은 색상각의 명도 92%다. 주제색과 값이 겹치는 칸이 있지만 뜻은 다르므로, 도표의 색으로 주제를 읽게 하지 않는다.

| 칸 | 테두리 | 채움 | 칸 | 테두리 | 채움 |
|---|---|---|---|---|---|
| 0 | `#5d7897` | `#e4eaf1` | 6 | `#9c6e6e` | `#f0e5e5` |
| 1 | `#9a7852` | `#f3ebe2` | 7 | `#4a8791` | `#e2f1f4` |
| 2 | `#5c806c` | `#e6efea` | 8 | `#996695` | `#f0e5ef` |
| 3 | `#a4617f` | `#f2e3ea` | 9 | `#658156` | `#e9f0e5` |
| 4 | `#9b8a45` | `#f4f0e1` | 10 | `#80698f` | `#ebe6ef` |
| 5 | `#67639c` | `#e5e4f1` | 11 | `#817f72` | `#ecece9` |
...
- **도표:** Mermaid는 필요한 페이지에서 지연 로딩한다. 배치 `dagre`, 외형 `neo`, 라벨 접힘 폭 `120px`, 노드 최소 폭 `60px`, 테마 `redux-color`를 기본으로 쓰며 값은 [mermaid-config.ts](src/scripts/mermaid-config.ts)에 모아 둔다. 색은 위 도표 팔레트와 사이트 값으로 덮고, 노드는 연두 한 가지로 두어 묶음과 항목이 여럿인 도표에만 팔레트를 준다. 앞머리에 `theme`을 적은 도표는 그 테마를 보여 주려는 것이므로 사이트 색을 입히지 않고 서체만 맞춘다. 본문 폭을 넘는 도표는 줄였을 때 라벨이 `13px` 이상 남으면 접어 넣고, 그보다 작아지면 원래 크기로 두고 가로로 스크롤한다. 스크롤이 생긴 도표에만 키보드 초점과 위 오른쪽 `크게 보기` 버튼을 붙이며 이름은 모든 도표에 붙인다. 모달은 전체 구조를 먼저 보여 주고 `원래 크기`로 세부를 보며, 조작은 보기 전환을 왼쪽, 닫기를 오른쪽 끝에 두고 `720px` 이하에서는 도표 아래로 내려간다. 도표를 렌더링하지 못하면 기존 코드 블록으로 원문을 표시한다.
- **도표 비교:** 같은 도표를 설정만 바꿔 견주는 자리는 `diagram-comparison` 격자로 감싸고 각 도표를 `diagram-case`로 묶는다. 칸은 최소 `20rem`이고 화면이 좁아지면 한 줄로 내려간다. 마크업은 노트 본문이 정하므로 형식은 [AUTHORING.md](AUTHORING.md)에 적어 둔다.
...
- **Don't** 의미 없는 장식 요소, 글로우, 장식용 그라디언트와 그림자를 기본값으로 추가하지 않는다. 이 기준은 본문과 UI에 적용한다. 도표의 노드 그림자는 도형을 배경과 구분해 주므로 예외로 두며, 가로 스크롤 60프레임 측정에서 그림자를 켜고 끈 프레임 시간이 같았다.
...
호버와 선택은 색·선·링으로 즉시 알아볼 수 있게 한다. 키보드 포커스는 전역 2px 윤곽선과 4px 간격으로 표시하고, 연재 링크와 그래프는 각자의 테두리·링을 사용한다. 본문과 조작 요소의 상태 변화는 150ms, 모바일 지도 패널의 열림·닫힘은 200ms, 그래프의 선택 이동은 320ms 안에서 처리한다. 그래프의 드래그·확대는 장식용 자동 모션이 아니라 사용자의 입력에 따른 조작이다. `prefers-reduced-motion`에서는 전환과 부드러운 스크롤을 끄고, 모바일에서는 조작 영역과 주변 간격을 함께 확인한다. 손가락으로 누르는 버튼은 글자 크기를 키우지 않고 누를 수 있는 높이를 `44px`로 잡는다. 이 값은 이 사이트가 정한 터치 목표이며, 플랫폼 지침의 pt·dp를 환산한 값이 아니다.
