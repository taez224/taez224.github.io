...
- 한국어로 출력되는 모든 결과물에 이 지침들을 적용합니다. 사용자에게 보고하는 문장뿐 아니라 저장소의 문서와 vault의 노트에도 적용합니다.

- 적용하지 않는 자리는 문서 첫머리의 인용과 코드, 코드 주석에 더해 변수명과 로그 문자열, 커밋 메시지까지입니다. 각자의 기존 관례를 그대로 따릅니다. 커밋 제목을 명사형으로 끝내는 규칙이 유지되는 것도 이 때문이며, 「문장 단위」 2번의 종결 조항을 커밋 메시지에 적용하지 않습니다.

- 적용 범위의 정본은 `AGENTS.md`의 「한국어 문체」 절입니다. 두 문서가 어긋나면 그쪽을 따릅니다.

- 한국어 사전에 있는 어휘이며 뜻이 명확하더라도, 사용 빈도가 낮아서 통용되지 않는 어휘를 사용하면 소통의 효율성이 오히려 낮아지니 자제합니다. 대신 의미가 명확하며 실제로 통용되는 어휘를 우선적으로 선택합니다.

- 위의 지침들은 문장 내부를 규율합니다. 문서의 분량과 구조, 종결체, 형식은 `AGENTS.md`의 「한국어 문체」 절과 각 산출물의 기존 규칙이 정하므로, 두 가지를 동시에 지킵니다. 분량을 줄이라는 요구는 문장 수를 줄이라는 뜻이지, 문장에서 조사와 어미를 덜어내라는 뜻이 아닙니다.

- 이 지침 본문이 "~합니다"체로 서술되어 있더라도, 산출물의 종결체는 그 산출물의 기존 규칙을 따릅니다. 이 저장소의 문서는 "~한다" 평서형으로 씁니다.

- vault(`../obsidian`)의 노트와 글을 쓸 때에는 그 저장소 `AGENTS.md`의 「글쓰기」 절과 각 스킬의 규칙을 따릅니다. 그 규칙들은 비유와 꾸민 표현을 이미 금지하므로 위 「구 단위」 3번과 어긋나지 않습니다.
