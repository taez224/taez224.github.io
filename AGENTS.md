...
- 픽셀 도안의 단일 출처는 `src/lib/mark.ts`(개인 표식)와 `src/lib/pixel-icons.ts`(조작 아이콘)의 격자다. 한 칸이 1px이므로 격자 칸 수와 표시 크기가 어긋나면 도안이 흐려지고, `tests/mark.test.ts`와 `tests/pixel-icons.test.ts`가 실패한다. 파비콘과 터치 아이콘은 `scripts/make-mark.ts`가 만드는 생성물이라 직접 고치지 않는다.
